...
const Map: React.FC = () => {
	...
	const updateBikeMarkers = (updateInput: LocationUpdateData | LocationUpdateData[]) => {
	...
	useEffect(() => {
		...
		const fetchBikes = async (cityName: string) => {
			try {
				const response = await fetch("http://localhost:1337/api/v1/get/all/bikes");
				if (!response.ok) {
					throw new Error(`Error: ${response.statusText}`);
				}
		...
		const fetchChargingStations = async (cityName: string) => {
			try {
				const response = await fetch(
					`http://localhost:1337/api/v1/get/city/${cityName}/charging-stations`
				);
				if (!response.ok) {
					throw new Error(`Error: ${response.statusText}`);
				}
		...
		const fetchParkingZones = async (cityName: string) => {
			try {
				const response = await fetch(
					`http://localhost:1337/api/v1/get/city/${cityName}/parking-zones`
				);
				if (!response.ok) {
					throw new Error(`Error: ${response.statusText}`);
				}

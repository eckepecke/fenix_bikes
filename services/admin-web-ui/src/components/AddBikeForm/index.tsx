...
const AddBikeForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const selectedCity = cityParkingZones.find(
      (city) => city.name === cityName
    );
    const selectedZone = availableZones.find(
      (zone) => zone._id === parkingZone
    );
    ...
    try {
      const response = await fetch("http://localhost:1337/api/v1/add/bike", {
        method: "POST",
        headers: {
      ...
      <form className="add-bike-form" onSubmit={handleSubmit}>
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              handleCityChange(e, formData, setFormData, setAvailableZones)
            }
            required
          >

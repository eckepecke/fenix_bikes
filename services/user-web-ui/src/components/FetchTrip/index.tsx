...
const FetchTrip = async (tripId: string): Promise<Trip | null> => {
	try {
		const tripResponse = await fetch(`http://localhost:1337/api/v1/get/trip/${tripId}`);
		if (!tripResponse.ok) {
		...
		const tripCostResponse = await fetch(`http://localhost:1337/api/v1/trip/calculate/${tripId}`);
